$(document).ready(function(){
	...
	var req_number;
	//댓글 목록
	function selectData(pageNum,req_num){
		currentPage = pageNum;
		req_number = req_num;
		...
		$.ajax({
			type:'post',
			data:{pageNum:pageNum,req_num:req_num},
			url:'listReply.do',
			dataType:'json',
			cache:false,
			timeout:30000,
			success:function(data){
				...
				if(count < 0 || list == null){
					...
					$(list).each(function(index,item){
						var accept_color = 'white';
						var accept_status = '';
						if(item.re_accept==1){
							accept_color = 'skyblue';
							accept_status = '<span style="color:red"><배송자로 선택></span>';
						}
						
						var output = '<div class="item" style="background-color:'+accept_color+'">';
						output += '  <h4>' + item.id + accept_status +'</h4>';
						output += '  <div class="sub-item">';
						//output += '    <p>' + item.re_content.replace(/\n/g,'<br>') + '</p>';
						output += '    <p>' + item.re_content.replace(/</gi,'&lt;').replace(/>/gi,'&gt;') + '</p>';
						output += item.re_date;
						if($('#user_id').val()==$('#writer_id').text()){
							output += ' <input type="checkbox" data-num="'+item.re_num+'" class="accept_request">';
							output += '<span class="accept-letter">승인</span>';
						}
	...
	$('.paging-button input').click(function(){
		...
		selectData(pageNum,$('#req_num').val());
	});
	...
	$(document).on('click','.accept_request',function(event){
		var choice_input = $(this);
		$('.accept_request').hide();
		$('.accept-letter').hide();
		var re_num = $(this).attr('data-num');
		
		$.ajax({
			url:'accept_request.do',
			type:'post',
			data:{re_num:re_num,req_num:req_number},
			dataType:'json',
			cache:false,
			timeout:30000,
			success:function(data){
				if(data.result=='logout'){
					...
					choice_input.parents('.item').css('background-color','skyblue');
					choice_input.parents('.item').find('h4').append('<span style="color:red"><배송자로 선택></span>');
					$('#reply_div').after('<div class="align-center"><b>배송자 선택이 완료되었습니다.</b></div><hr size="1" width="100%">');
					$('#reply_div').hide();
				}else{
					alert('수정 오류 발생');
				}
